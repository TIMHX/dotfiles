...
description: |
  ...
  voice, filler, or chatbot artifacts. Handles LaTeX/.tex manuscripts safely.
...
## LaTeX and markup safety

When the text is LaTeX (a `.tex` file, or a pasted fragment containing backslash commands), rewrite only the prose. Everything below is structure, not writing, and must survive the rewrite byte for byte.

**Never change:**
- Math: `$...$`, `$$...$$`, `\(...\)`, `\[...\]`, and the bodies of `equation`, `align`, `gather`, `split`, `array`, `matrix`, and their starred forms.
- Citation and cross-reference keys: the argument of `\cite`, `\citep`, `\citet`, `\ref`, `\eqref`, `\autoref`, `\label`, `\bibitem`. Never invent, drop, merge, or renumber a key.
- Command and environment names, and any non-text arguments: `\begin{...}`, `\end{...}`, `\includegraphics[...]{...}`, `\usepackage`, `\newcommand`, lengths, keys, and file paths.
- Verbatim content: `verbatim`, `lstlisting`, `minted`, `\verb`, `tabular` column specs, and TikZ/pgfplots bodies.
- The preamble, i.e. everything before `\begin{document}`.

**Safe to rewrite:** body paragraphs, the argument text of `\section`, `\subsection`, `\title`, `\caption`, `\footnote`, `\emph`, `\textbf`, and the prose inside `abstract`, `itemize`, `enumerate`, and `description` items.

**Rules while rewriting:**
- Keep every `\cite`/`\ref` attached to the claim it supported. If merging or splitting sentences moves a claim, move its citation with it.
- Do not turn an inline math symbol into a word, or a word into math. `$n$ samples` stays `$n$ samples`.
- Keep escaped characters escaped: `%`, `&`, `_`, `#`, `$`, `~`, `^`, `\\`. A literal `%` must stay `\%`; do not introduce a bare `%`, which would comment out the rest of the line.
- Leave `%` comment lines alone unless the user asks otherwise.
- Do not add, remove, or reorder `\label` commands, and do not renumber anything by hand — LaTeX numbers floats, equations, and sections itself.
- Preserve non-breaking ties (`Fig.~\ref{...}`) and do not replace `~` with a space.
- Do not introduce Unicode that the document's engine may not support (typically em dashes, curly quotes, or `≤`) unless the source already uses it. Use `---`, `` `` ''``, and `$\le$` instead.

If a rewrite would require touching any of the protected items above, leave that sentence as it is and mention it in the summary instead.
...
**File mode.** When the user names a file, run the full rewrite process but write only the final text to the file. Change prose only. Keep code blocks, YAML metadata, data, and link targets unchanged. For `.tex` files, follow [LaTeX and markup safety](#latex-and-markup-safety). Then give the user a short summary.
